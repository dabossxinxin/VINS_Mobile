...
Vector4f creatPlane(Vector3f p1,Vector3f p2, Vector3f p3)
{
    ...
    Eigen::Vector3f normal = arrow1.cross(arrow2);
    float normalz = -normal.x() * p1.x() - normal.y() * p1.y() - normal.z() * p1.z();
    return Vector4f(normal.x() , normal.y(), normal.z(), normalz);
}
...
void DrawResult::Reprojection(cv::Mat &result, vector<Vector3f> &point_cloud, const Matrix3f *R_window,
                              const Vector3f *T_window, bool box_in_trajectorty)
{
    float depth_marker;
    result = cv::Mat(WIDTH, HEIGHT, CV_8UC3, Scalar(242,242,242));
    
    Eigen::Matrix3f RIC = Utility::ypr2R(Vector3d(RIC_y,RIC_p,RIC_r)).cast<float>();
    Eigen::Matrix3f R_v_c = Utility::ypr2R(Eigen::Vector3f{yaw, pitch, roll});
    Eigen::Vector3f T_v_c = Eigen::Vector3f{Tx,Ty,Tz};
