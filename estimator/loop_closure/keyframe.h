...
{
public:
    KeyFrame(double _header, int _global_index, Eigen::Vector3d _T_w_c, Eigen::Matrix3d _R_w_c,
             cv::Mat &_image, const char *_brief_pattern_file, const int _segment_index);
    void setExtrinsic(Eigen::Vector3d T, Eigen::Matrix3d R);
    void initPtsByReprojection(Eigen::Vector3d Ti_predict, Eigen::Matrix3d Ri_predict,
                               std::vector<cv::Point2f> &measurements_predict);
    void initPoseForPnP(Eigen::Vector3d &T_c_w, Eigen::Matrix3d &R_c_w);
    void cam2Imu(Eigen::Vector3d T_c_w, Eigen::Matrix3d R_c_w,
                 Eigen::Vector3d &T_w_i, Eigen::Matrix3d &R_w_i);
    void rejectWithF(vector<cv::Point2f> &measurements_old,
                     vector<cv::Point2f> &measurements_old_norm);
    void extractBrief(cv::Mat &image);
    void searchInBoW(std::vector<cv::Point2f> &cur_pts, std::vector<cv::Point2f> &old_pts,
                     std::vector<cv::Point2f> &old_measurements);
    void buildKeyFrameFeatures(VINS &vins);
    
    void searchByDes(std::vector<cv::Point2f> &measurements_old, std::vector<cv::Point2f> &measurements_old_norm,
                     const std::vector<BRIEF::bitset> &descriptors_old, const std::vector<cv::KeyPoint> &keypoints_old);
    
    bool solveOldPoseByPnP(std::vector<cv::Point2f> &measurements_old_norm, const Eigen::Vector3d T_w_i_old,
                           const Eigen::Matrix3d R_w_i_old, Eigen::Vector3d &T_w_i_refine, Eigen::Matrix3d &R_w_i_refine);
    
    bool findConnectionWithOldFrame(const KeyFrame* old_kf, const std::vector<cv::Point2f> &cur_pts,
                                    const std::vector<cv::Point2f> &old_pts, std::vector<cv::Point2f> &measurements_old,
    ...
    std::list<pair<int, Eigen::Matrix<double, 8, 1 > > > connection_list;
