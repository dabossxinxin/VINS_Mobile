...
void vinsPnP::updateFeatures(std::vector<IMG_MSG_LOCAL> &feature_msg)
{
...
void vinsPnP::new2old()
{
    ...
    for (int i = 0; i <= PNP_SIZE; i++)
    {
        if (1) {
            ...
            Vs[i] = rot_diff * Vector3d(para_Speed[i][0],para_Speed[i][1],para_Speed[i][2]);
        ...
        Bas[i] = Vector3d(para_Bias[i][0],para_Bias[i][1],para_Bias[i][2]);
        Bgs[i] = Vector3d(para_Bias[i][3],para_Bias[i][4],para_Bias[i][5]);
